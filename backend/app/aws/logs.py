...
async def delete_log_group(log_group_name: str) -> None:
    async with get_logs_client() as client:
        try:
            await client.delete_log_group(logGroupName=log_group_name)
        except ClientError as e:
            error_code = e.response.get("Error", {}).get("Code", None)
